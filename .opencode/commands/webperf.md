...
- A live URL plus the chrome-devtools MCP server configured in the harness (capture metrics directly via lighthouse*audit and performance*\* tools)
...
- A note on which mode is expected (Quick or Deep), so missing inputs surface if Deep was intended
