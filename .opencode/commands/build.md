...
Skills: .opencode/skills/incremental-implementation/SKILL.md and .opencode/skills/test-driven-development/SKILL.md
...
1. Require a spec. Look only for a spec at .opencode/planning/SPEC.md. A README or arbitrary doc does NOT count. If none exists, stop and tell the user to run /spec first — do not invent requirements.
2. Establish a clean baseline. Run `git status --porcelain`. If there are uncommitted changes outside the expected planning artifacts (.opencode/planning/SPEC.md, .opencode/planning/tasks/plan.md, .opencode/planning/tasks/todo.md), stop and ask the user to commit, stash, or confirm how to handle them. Autonomous per-task commits must not absorb unrelated local work, or the clean-rollback guarantee breaks.
3. Plan if needed. If there is no .opencode/planning/tasks/plan.md, invoke the planning-and-task-breakdown skill to generate one.
4. Single checkpoint. Present the full plan and wait for an unambiguous affirmative (e.g. "approve", "go", "yes"). Treat hedged responses ("looks reasonable", "I guess") as NOT approved. This is the only human gate — after approval, run autonomously. If you generated .opencode/planning/tasks/plan.md, commit it as a single preparatory commit now so it doesn't bleed into the first task's commit.
